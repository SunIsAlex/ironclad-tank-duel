...
- **局间军械商店**：每局双方获得 700 点，胜者额外获得 350 点；15 种特殊武器分为战地、进阶、精英、原型四档，价格从 300 到 4200，点数与剩余弹药跨局保留，AI 也会为高阶军械存点
- **扩展武器机制**：除反弹、钻地、滚轮、空袭和主动集束外，新增放射碎片、空中弹雨、地表震波、五联扇射与终局级巨型爆炸
...
- **地形破坏方案**：
  1. 全分辨率二值实体数据作为碰撞与破坏的唯一真源
  2. 离屏 Canvas 仅负责地形显示，爆炸使用同一弹坑轮廓同步擦除
  3. 1D `heightMap` 缓存每列最上层表面，爆炸后只重建受影响列
  4. 多次爆炸以逐像素幂等清除叠加，不产生粗网格隐形坑沿
  5. 坦克用整段履带多点接触求解高度和车身角度，可跨窄缝但会落入宽坑
  6. 地形被炸空后立即进入连续重力结算，移动采用亚像素步长和平滑姿态
- **移动端适配方案**：
